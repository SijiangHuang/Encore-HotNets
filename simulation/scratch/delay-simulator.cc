...
std::string link_output_file = "link.txt";
std::string tm_output_file = "traffic_matirx.txt";
...
std::unordered_map<uint32_t, std::vector<uint32_t> > flowLink;
...
std::unordered_map<uint32_t, uint64_t> flowDelay;
// std::vector<uint32_t> flowCount;
// std::vector<float> flowDelay;
std::unordered_map<uint32_t, std::set<uint32_t> > nodeLink;
std::vector<uint32_t> hostTM;
std::vector<uint32_t> linkUsage;
uint32_t timeSlice = 0;

// std::unordered_map<uint32_t, std::vector<uint32_t> > hostTM;
// std::unordered_map<uint32_t, std::vector<uint32_t> > switchTM;
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		// flowLink[flow_input.idx].push_back(flow_input.src);
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	fprintf(fout, "%u,%lu,%u,%u,%lu,%lu,%lu,%.2ld\n", fid, q->m_size, sid, did, q->startTime.GetTimeStep(), (Simulator::Now() - q->startTime).GetTimeStep(), standalone_fct, flowDelay[fid] / q->m_size);
	fflush(fout);
	flowFinished++;
}
...
void qbb_dequeue(Ptr<QbbNetDevice> dev, Ptr<const Packet> pkt, uint32_t i)
{
	...
	uint32_t fid = flowMap[src][dst][sport];
	uint32_t node = dev->GetNode()->GetId();
	uint32_t port = dev->GetIfIndex();
	uint32_t link = node * switch_port + port - 1;
	...
	if(sport >= 10000)
	{
		// std::cout << "node[" << node << "] port[" << port << "] link[" << link << "]\n";
		if(linkInFlow[fid].find(link) == linkInFlow[fid].end())
		{
			linkInFlow[fid].insert(link);
			flowLink[fid].push_back(link);
		}
		if(nodeInFlow[fid].find(node) == nodeInFlow[fid].end())
		{
			nodeInFlow[fid].insert(node);
			flowNode[fid].push_back(node);
		}
	} 
}
...
void host_send(uint32_t i, Ptr<const Packet> pkt)
{
	...
	uint32_t fid = flowMap[src_id][dst_id][port_id];
	uint32_t pkt_seq = ch.udp.seq / packet_payload_size;
	
	pktStartTime[fid][pkt_seq] = Simulator::Now().GetNanoSeconds();
	uint32_t size = pkt->GetSize();
	hostTM[src_id * host_num + dst_id] += size;

	// std::cout << "send, src_id:" << src_id << " dst_id:" << dst_id << " pkt_seq:" << pkt_seq << " time:" << Simulator::Now().GetNanoSeconds() << std::endl;
}
...
void host_receive(uint32_t i, Ptr<const Packet> pkt, CustomHeader ch)
{
	...
	uint32_t fid = flowMap[src_id][dst_id][port_id];
	uint32_t pkt_seq = ch.udp.seq / packet_payload_size;
	uint64_t pkt_delay = (Simulator::Now().GetNanoSeconds() - pktStartTime[fid][pkt_seq]);

	// if(linkInFlow[fid].find(dst_id) == linkInFlow[fid].end())
	// {
	// 	linkInFlow[fid].insert(dst_id);
	// 	flowLink[fid].push_back(dst_id);
	// }
	
	flowDelay[fid] += pkt_delay * pkt->GetSize();

	// std::cout << "receive, flow_id:" << flow_id << " pkt_seq:" << pkt_seq << " delay:" << flowDelay[flow_id] << " " << pkt_delay << std::endl;
}
...
void PrintTime()
{
	...
	// {
	// 	hostTM[timeSlice] = hostSlice;	
	// 	switchTM[timeSlice] = switchSlice;	
	// 	timeSlice++;

	// 	hostSlice = std::vector<uint32_t>(host_num * host_num, 0);
	// 	switchSlice = std::vector<uint32_t>(node_num * 4, 0);

	// 	Simulator::Schedule(Seconds(0.001), PrintTime);
	// 	if(flowFinished == flow_num)
	// 	{
	// 		flowFinished++;
	// 	}
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			else if (key.compare("LINK_OUTPUT_FILE") == 0){
				conf >> link_output_file;
			}
			else if (key.compare("TM_OUTPUT_FILE") == 0){
				conf >> tm_output_file;
			}
	...
	hostTM = std::vector<uint32_t>(host_num * host_num, 0);
	linkUsage = std::vector<uint32_t>(node_num * switch_port, 0);
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		if(logPath)
		{
			// DynamicCast<QbbNetDevice>(d.Get(0))->TraceConnectWithoutContext("Header", MakeBoundCallback (&qbb_header, i, DynamicCast<QbbNetDevice>(d.Get(0))));
			// DynamicCast<QbbNetDevice>(d.Get(1))->TraceConnectWithoutContext("Header", MakeBoundCallback (&qbb_header, i, DynamicCast<QbbNetDevice>(d.Get(1))));

			DynamicCast<QbbNetDevice>(d.Get(0))->TraceConnectWithoutContext("QbbDequeue", MakeBoundCallback (&qbb_dequeue, DynamicCast<QbbNetDevice>(d.Get(0))));
			DynamicCast<QbbNetDevice>(d.Get(1))->TraceConnectWithoutContext("QbbDequeue", MakeBoundCallback (&qbb_dequeue, DynamicCast<QbbNetDevice>(d.Get(1))));
		}
	...
	FILE *link_output = fopen(link_output_file.c_str(), "w");
	FILE *tm_output = fopen(tm_output_file.c_str(), "w");
	fprintf(fct_output, "fid,size,src,dst,starttime,fct,standalone_fct,delay\n");
	//
	// install RDMA driver
	//
	for (uint32_t i = 0; i < node_num; i++){
	...
	// Simulator::Schedule(Seconds(2) , PrintTime);
	topof.close();
	...
	// fprintf(link_output, "link,value\n");
	// for(uint i = host_num * switch_port;i < node_num * switch_port;i++)
	// {
	// 	fprintf(link_output, "%u,%u\n", i, linkUsage[i]);
	// }
	// fclose(link_output);

	// fprintf(tm_output, "host_pair,value\n");
	// for(uint i=0;i<host_num * host_num;i++)
	// {
	// 	fprintf(tm_output, "%u,%u\n", i, hostTM[i]);
	// }
	// fclose(tm_output);

	// // // for (auto it:linkUsage)
	// // // {
	// // // 	std::cout << it << " ";
	// // // }
	// // // std::cout << std::endl;
